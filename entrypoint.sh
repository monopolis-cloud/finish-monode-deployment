...
if jq --exit-status '.inputs.ROLLOUT_DEPLOYMENT_ID' "$GITHUB_EVENT_PATH" >/dev/null; then
  MONOPOLIS_URL="https://api.monopolis.cloud/v1/rollout/finish/$(get_from_event '.repository.full_name')/$(get_from_event '.inputs.ROLLOUT_DEPLOYMENT_ID')/${INPUT_SUCCESS}"
